...
const Timeline = ({ entries }: { entries: ITimelineEntry[] }) => {
  return (
    <section className="flex overflow-hidden flex-col items-center px-4 py-8 w-full max-w-none bg-white prose-headings prose-p prose dark:bg-slate-900 dark:prose-invert">
      <h1>Projects</h1>
      {entries.map((entry, idx) => {
        // if (["Persistence"].includes(entry.title)) {
        return (
...
const TimelineCard = ({
  ...
  return (
    <div
      ref={containerRef}
      className={`grid-auto-rows my-4 mx-0 grid w-10/12
      transform grid-cols-5 items-center rounded-3xl p-4 px-0 transition duration-500 
      ${
        isVisible || index === 0
          ? "-transform-x-1/2 bg-gradient-to-tr from-red-200 via-slate-100 to-blue-100 opacity-100 shadow-xl dark:from-red-300 dark:via-slate-200 dark:to-blue-300"
    ...
    >
      ...
      <h6
        ...
        dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(entry.brief) }}
      ></h6>
    ...
    <div className="flex flex-col justify-evenly h-full prose">
      {Object.entries(props).map(([name, text]) => (
        <div key={id + name + "-container"}>
          <h5 key={id + name + "-title"} className="italic font-bold">
            {name}
          </h5>
          <p
            key={id + name + "-text"}
...
const ProjectStatus = (props: ITimelineEntryStatus) => {
  return (
    <div className="flex flex-col justify-evenly pl-4 h-full border-l prose border-slate-400 dark:border-slate-600">
      {Object.entries(props).map(([name, text]) => (
        <div key={name}>
